import { revalidatePages } from '@/lib/api/server/revalidationHandler';
import { NextResponse } from 'next/server';
import { Session } from 'next-auth';
import { getUserById, updateUserStatus } from '@/lib/api/db';
import { UserRole } from '@/lib/types/user';
import { withAuthParam } from '@/lib/api/server/routeHandlers';

async function updateStatusHandler(request: Request, session: Session, params: Promise<{ id: string }>): Promise<NextResponse> {
	const { id } = await params;

	if (id === session.user.id) {
		return NextResponse.json(
			{ error: 'You cannot change your own account status' },
			{ status: 403 }
		);
	}

	const targetUser = await getUserById(id);
	if (!targetUser) {
		return NextResponse.json(
			{ error: 'User not found' },
			{ status: 404 }
		);
	}

	const roleRanks: { [key in UserRole]: number } = {
		'admin': 4,
		'president': 3,
		'vice_president': 2,
		'secretary': 1,
		'holdings_write': 1,
		'holdings_read': 1,
		'user': 0
	};

	const currentUserRank = roleRanks[session.user.role as UserRole];
	const targetUserRank = roleRanks[targetUser.role as UserRole];
	if (targetUserRank >= currentUserRank) {
		return NextResponse.json(
			{ error: 'You cannot change the status of users with equal or higher rank' },
			{ status: 403 }
		);
	}

	const data = await request.json();
	const isActive = data.isActive as boolean;
	if (typeof isActive !== 'boolean') {
		return NextResponse.json(
			{ error: 'Invalid status specified' },
			{ status: 400 }
		);
	}

	const success = await updateUserStatus(id, isActive);
	if (!success) {
		return NextResponse.json(
			{ error: 'Failed to update user status' },
			{ status: 500 }
		);
	}

	revalidatePages('user');
	return NextResponse.json({
		message: `User ${isActive ? 'activated' : 'deactivated'} successfully`,
		isActive
	});
}

export const PUT = withAuthParam(updateStatusHandler, 'ADMIN');