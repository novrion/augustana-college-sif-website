...
import { formatDateForDisplay, formatCurrency, formatFileSize } from '@/lib/utils';

export default async function PitchDetail({ params }: { params: Promise<{ id: string }> }) {
	const session = await getSession();
	if (!session) { redirect('/login?callbackUrl=/pitches'); }

	const hasAccess = await hasPermission('HOLDINGS_READ');
	if (!hasAccess) redirect('/unauthorized');

	try {
		const { id } = await params;
		if (!id) redirect('/pitches');
				...
				<div className="max-w-4xl mx-auto">
					...
					<div className="rounded-lg border border-solid border-white/[.145] p-6">
						<div className="flex items-start justify-between mb-4">
							<h2 className="text-2xl font-semibold font-[family-name:var(--font-geist-mono)]">
								{pitch.title}
							</h2>
							<span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${pitch.is_buy
						...
						<div className="flex flex-col sm:flex-row sm:items-center text-sm text-gray-400 mb-6 font-[family-name:var(--font-geist-mono)]">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
							...
							<div>
								...
								<p className={`font-medium ${pitch.is_buy ? 'text-green-400' : 'text-red-400'}`}>
									{pitch.is_buy ? 'BUY' : 'SELL'}
								</p>
							</div>
							<div>
								<span className="text-sm text-gray-400">Analyst</span>
								<p className="font-medium">{pitch.analyst}</p>
							</div>
						</div>

						{pitch.description && (
							<div className="mb-8">
								<h3 className="text-lg font-semibold mb-2 font-[family-name:var(--font-geist-mono)]">Analysis</h3>
								<div className="prose prose-sm max-w-none font-[family-name:var(--font-geist-sans)]">
						...
						{pitch.attachments && renderAttachments(pitch.attachments)}
					</div>
				</div>
			</div>
		);
...
function renderImageAttachments(attachments: Attachment[]) {
	return (
		<div className="mb-8 font-[family-name:var(--font-geist-mono)]">
			<h3 className="text-lg font-semibold mb-4">Supporting Images</h3>
